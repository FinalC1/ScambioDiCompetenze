    ...
    fetch(`/api/lezioni/${lessonId}`)
        ...
        .then(data => {
            if (data.ok) {
                ...
                alert('Errore: Errore caricamento dettagli lezione');
                window.location.href = '/dashboard';
            }
        })
        .catch(err => {
            console.error('Errore:', err);
            alert('Errore caricamento dettagli lezione');
            window.location.href = '/dashboard';
        });
}
  ...
  `;
    ...
    const partecipantiHtml = data.partecipanti.length > 0 ?
        ...
        <div class="participant">
          <div class="part-av">${p.username.charAt(0).toUpperCase()}</div>
          <div>
            <div class="part-name">@${p.username}</div>
...
function renderActionButton(data) {
    ...
    if (data.is_teacher) {
      ...
      <button class="btn-contact" onclick="contactTeacher('${data.lezione.id_ins}', '${data.lezione.nome_insegnante}')">
