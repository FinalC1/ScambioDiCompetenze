// Variabili globali per socket_chat.js
  ...
  function renderConvList(conversations, filter = '') {
    ...
    el.innerHTML = vis.map(c => {
      const prev = c.ultimo_msg ? (c.mittente_id === myUserId ? 'Tu: ' : '') + c.ultimo_msg : '';
      return `<div class="conv-item${c.id_utente === window.activeChatId ? ' active' : ''}" 
        ...
        <div class="av-wrap">
          ...
          ${c.online ? '<div class="dot-online"></div>' : ''}
        </div>
        <div class="conv-info">
          <div class="conv-top">
  ...
  window.updateConvPreview = function(userId, testo, ora) {
    ...
    items.forEach(item => {
      ...
      if (prevEl) prevEl.textContent = testo;
    ...
    fetch(`/api/messaggi/chat/${userId}`)
      ...
      .then(data => {
        if (data.ok) {
          ...
          console.error('Errore chat:', data.error);
        }
      })
