...
DB_URL = os.environ.get(
    'DB_URL',
    'postgresql://postgres:PASSWORD@HOST:6543/postgres' # Usa la porta 6543 di Supabase per il pooler su Render
)
...
# CHIAVE DI RISOLUZIONE INVIO EMAIL E OTP:
# Utilizzo corretto della libreria MIMEText per superare i controlli SMTP di Google.
...
def api_login():
    ...
    try:
        ...
        print(f'[LOGIN ERROR] {e}'); return err('Errore interno del server durante il login')
...
def api_register():
    ...
    try:
        ...
        send_email(email,'Benvenuto in SkillBridge!',
            ...
            f'Usa il codice univoco per farti trovare dagli altri studenti in chat.\n\nIl team di SkillBridge')
        return ok(message='Registrazione completata con successo!')
    except Exception as e:
        conn.rollback()
        print(f'[REGISTRATION ERROR] {e}')
        return err('Email già registrata o errore del server')
...
# ─────────────────────────────────────────────────────────────────────────────
# API: PASSWORD RESET (OTP via email)
# ─────────────────────────────────────────────────────────────────────────────
...
def api_reset_richiedi():
    ...
    send_email(email,'SkillBridge — Codice verifica reset password',
        f"Ciao {user['nome']},\n\nIl tuo codice di verifica per reimpostare la password è: {codice}\n"
        f"Questo codice scadrà tra 15 minuti.\n\nSe non hai richiesto tu il reset, ignora questo messaggio.")
    return ok(token=token, message='Codice inviato con successo!')
...
def api_reset_nuova():
    ...
    if len(pw)<6: return err('La password deve essere di minimo 6 caratteri')
    conn=get_db(); cur=conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute("SELECT * FROM reset_password WHERE token=%s AND usato=FALSE AND scadenza>NOW()",(token,))
    row_=cur.fetchone()
    ...
    conn.commit(); cur.close(); conn.close()
    return ok(message='Password aggiornata correttamente!')
...
def api_crea_lezione():
    ...
    try:
        ...
        conn.rollback(); print(f'[CREATE LESSON ERROR] {e}'); return err('Errore nella creazione della lezione')

# BUG DI DETTAGLIO LEZIONE RISOLTO:
# Il backend mappa 'titolo' come 'nome_materiale' per garantire la compatibilità con lesson.js
...
def api_lezione_detail(id):
    ...
    # query rimappata con Alias per compatibilità JS
...
def api_prenota(id):
    ...
    if cur.fetchone(): cur.close(); conn.close(); return err('Già prenotato a questa lezione')
    ...
    cur.execute("INSERT INTO prenotazione (data_prenotazione,stato,id_utente,id_lezione) VALUES (%s,'Confermata',%s,%s)",
                (date.today(),uid,id))
    conn.commit(); cur.close(); conn.close()
    return ok(message='Prenotazione confermata con successo!')
...
def api_annulla(id):
    ...
    conn.commit(); cur.close(); conn.close()
    return ok(message='Prenotazione annullata con successo.')
...
# CHIAVE DI RISOLUZIONE VISUALIZZAZIONE "Tu:" IN CHAT LIST:
# Questa query ora seleziona anche 'ultimo_mittente_id' in modo che messaggi.js possa identificare il mittente.
...
# ─────────────────────────────────────────────────────────────────────────────
# WEBSOCKET CHAT LOGIC (Flask-SocketIO + Gevent)
# ─────────────────────────────────────────────────────────────────────────────
@socketio.on('connect')
def on_connect():
    if 'user_id' not in session:
    ...
    print(f'[WS] Connessione riuscita. Utente {uid} è online.')
...
def on_messaggio_privato(data):
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO messaggio (id_mittente,id_destinatario,contenuto) "
            "VALUES (%s,%s,%s) RETURNING id_messaggio, TO_CHAR(data_invio,'HH24:MI') AS ora",
        ...
        # Evento inviato a entrambi i socket dei client in ascolto (mittente e destinatario)
